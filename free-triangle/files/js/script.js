...
(function(window) {
	...
	class Triangle {
		...
		constructor(size,x,y,color) {
			...
			this.velocity   = getRangeNumber(1,3);
		...
		update(minX,minY,maxX,maxY) {
			...
			if (maxX <= this.center.x + this.size * .5) {
				this.direction.x = -1;
				this.velocity   = getRangeNumber(1,3);
			}

			if (this.center.x - this.size * .5 <= minX) {
				this.direction.x = 1;
				this.velocity   = getRangeNumber(1,3);
			}

			if (maxY <= this.center.y + this.size * .5) {
				this.direction.y = -1;
				this.velocity   = getRangeNumber(1,3);
			}

			if (this.center.y - this.size * .5 <= minY) {
				this.direction.y = 1;
				this.velocity   = getRangeNumber(1,3);
			}
			...
			this.center.x += this.velocity * this.direction.x;
			this.center.y += this.velocity * this.direction.y;
		...
		createVertexs(length) {
			...
			for (let i = 0; i < length; i++) {
				...
				const point = this.getCircleOnPoint(this.size,randomDistanse * i);
				points.push(point);
	...
	function setup() {
		...
		for (let i = 0; i < _setting.get('length'); i++) {
			...
			_objects.push(new Triangle(size,width*.5,height*.5,color));
